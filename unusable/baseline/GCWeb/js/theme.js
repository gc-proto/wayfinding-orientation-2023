...
function(q, T, N) {
    ...
    var z = "wb-fieldflow",
        ...
        n = [
            ["redir", "query", "ajax", "addClass", "removeClass", "removeClass", "append", "tblfilter", "toggle"].join("." + B + " ") + "." + B, ["ajax", "toggle", "redir", "addClass", "removeClass"].join("." + R + " ") + "." + R, ["tblfilter", z].join("." + M + " ") + "." + M, ["select", "checkbox", "radio"].join("." + Q + " ") + "." + Q
        ].join(" "),
        Y = function(e, t, a, r) {
            var n = e.data(t);
        ...
        X = function(e, t, a, r, n) {
            ...
            return s = r ? "<label class='" + a + (r ? "-inline" : "") + "'" + s : "<div class='" + a + "'><label" + s, s += l(e), n && (s += " required='required'"), s += " /> " + i + "</label>", r || (s += "</div>"), s
        };
    r.on(F, u + ", ." + v, function(e) {
        ...
        for (y && (n = (r = q("#" + v[y - 1])).data(H), q("#" + n[n.length - 1]).trigger(P), r.trigger(P)), a = 0; a !== y; a += 1)
            for (c = (i = (r = q("#" + v[a])).data(H)).length, d = 0; d !== c; d += 1) {
                ...
                if (h)
                    for (u = 0, p = h.length; u !== p; u += 1)(f = h[u]).form = g, r.trigger(f.action + "." + R, f), x.push({
                        $elm: r,
                        data: f
        ...
        for (y = x.length, a = 0; a !== y; a += 1)(f = x[a]).data.lastProvEvt = b, f.$elm.trigger(f.data.action + "." + U, f.data);
    ...
    }), r.on(n, u, function(e, t) {
        var a, r, n, i, o, s, l, d, c, u, p, f, h = e.type;
        switch (e.namespace) {
            case M:
                switch (h) {
                    case z:
                        ! function(e, t) {
                            if (e.namespace === M) {
                                ...
                                u.hasClass(v) && (a = N.getData(u, z), u.data($, a), t = q.extend({}, t, a)), s = t.actions || [], l = t.renderas ? t.renderas : "select", p.id || (p.id = N.getId()), (n = u.children().first()).hasClass(b) ? (i = n.html(), h = "." + b + " + " + h) : (i = (r = n.find(f)).length ? r.html() : u.find("> p").html(), f = null), o = x(u.find(h)), t.outputctnrid || (t.outputctnrid = t.provEvt.parentElement.id), c.trigger(l + "." + Q, {
                                    actions: s,
                                    source: p,
            ...
            case Q:
                switch (h) {
                    case "select":
                        ...
                        break;
                    case "checkbox":
                        t.typeRadCheck = "checkbox", y(e, t);
                        break;
                    case "radio":
                        t.typeRadCheck = "radio", y(e, t)
                }
                break;
            case B:
                switch (h) {
                    ...
                    case "redir":
                        ...
                        break;
                    case "ajax":
                        c = e, f = (u = t).provEvt, u.live ? (u.container || (p = q("<div></div>"), q(f.parentNode).append(p), u.container = p.get(0)), q(c.target).trigger("ajax." + R, u)) : (u.preventSubmit = !0, Y(q(f), _, u));
                        break;
                    case "tblfilter":
                        ! function(e, t) {
                            if (e.namespace === B) {
                        ...
                        break;
                    case "toggle":
                        t.live ? m(e, t) : (t.preventSubmit = !0, Y(q(t.provEvt), _, t));
                        break;
                    case "addClass":
            ...
            case R:
                switch (h) {
                    case "redir":
                        l = (s = t).form, (d = s.url) && l.setAttribute("action", d);
                        break;
                    case "ajax":
                        o = (a = t).clean, a.container ? r = q(a.container) : (n = N.getId(), r = q("<div id='" + n + "'></div>"), q(a.form).append(r), o = "#" + n), o && q(a.origin).one(P, function() {
                            q(o).empty()
                        }), a.trigger && r.attr("data-trigger-wet", "true"), i = a.type ? a.type : "replace", r.attr("data-ajax-" + i, a.url), r.one("wb-contentupdated", function(e, t) {
                            var a = e.currentTarget,
                                r = a.getAttribute("data-trigger-wet");
                        ...
                        }), r.trigger("wb-update.wb-data-ajax");
                        break;
                    case "toggle":
                        m(e, t);
                        break;
    ...
    }), r.on("timerpoke.wb " + a, u, function(e) {
        switch (e.type) {
            case "timerpoke":
            case "wb-init":
                ! function(e) {
